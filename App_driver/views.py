...
def print_case_details(request, case_id):
    case = CaseModel.objects.get(id=case_id)
    profile = DriverModel.objects.get(user=request.user)
    content = {
        'case': case,
        'profile': profile,
    }
    return render(request, 'App_driver/print_case_details.html', context=content)
